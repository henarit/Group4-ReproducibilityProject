...
OUTPKL = f"results/{MULTIMODAL}_mirna_clinical.pkl"
print (OUTFILE)
...
class Network(TrainableModel):
    ...
    def forward(self, data, mask):
        ...
        # mean interpretaton: for each feature and patient .. what is the mean feature
        # value across modalities clin/rnaseq/mirna?
        mean = masked_mean((x, y), (mask["mirna"], mask["clinical"])) # torch.Size([64, 256])

        # var interpretation:
        # for each feature and patient ... what is the variance of the feature 
        # value across modalities clin/rnaseq/mirna? var = mean of this variance 
        # across all 256 features and 64 cases in batch
        # IN SHORT: var is the average 'modality variance'
        var = masked_variance((x, y), (mask["mirna"], mask["clinical"])).mean() # single number

        # var2 interpretation: subtract mean --> can calculate variance like this...
        # for each feature and patient ... what is the variance of the feature 
        # value across modalities clin/rnaseq/mirna?
        var2 = masked_mean (((x - mean.mean())**2, (y - mean.mean())**2), \
                            (mask["mirna"], mask["clinical"])) # torch.Size([64, 256])

        

        # var2.mean(dim=1) interpretation: average 'modality variance' over all features.
        # ... one number per patient in the batch
        # var  = 'average modality variance across all features and patients in batch'
        # ... one number for the whole batch

        # ratios[i] is small if patient_i has feature vectors that varies a lot across modalities
        # (high variance) but looking at the full batch, the modality-variance is small on average
        # WHAT DOES THIS MEAN? And why do we want to minimize it?
        ratios = var/var2.mean(dim=1) # torch.Size([64])
        # Note:  var/var2.mean(dim=1) == var/(var2.mean(dim=1)) and torch.Size([64])

        ratio = ratios.clamp(min=0.02, max=1.0).mean() # single number approx 0.1-0.3
    ...
    def loss(self, pred, target):
        ...
        # convert probability --> log probabilitys
...
if __name__ == "__main__":
  ...
  model.save(f"results/predict1_{MULTIMODAL}.pth")
